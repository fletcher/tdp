...
**                       token.
**    TDPParseTOKENTYPE     is the data type used for minor type for terminal
...
**                       which is TDPParseTOKENTYPE.  The entry in the union
**                       for terminal symbols is called "yy0".
**    YYSTACKDEPTH       is the maximum depth of the parser's stack.  If
**                       zero the stack is dynamically sized using realloc()
**    TDPParseARG_SDECL     A static variable declaration for the %extra_argument
**    TDPParseARG_PDECL     A parameter declaration for the %extra_argument
**    TDPParseARG_STORE     Code to store %extra_argument into yypParser
**    TDPParseARG_FETCH     Code to extract %extra_argument from yypParser
...
#define TDPParseTOKENTYPE  simple_token *
typedef union {
	int yyinit;
	TDPParseTOKENTYPE yy0;
} YYMINORTYPE;
#ifndef YYSTACKDEPTH
	#define YYSTACKDEPTH 100
#endif
#define TDPParseARG_SDECL  simple_token ** result ;
#define TDPParseARG_PDECL , simple_token ** result
#define TDPParseARG_FETCH  simple_token ** result  = yypParser->result
...
static const YYCODETYPE yyFallback[] = {
	0,  /*          $ => nothing */
	0,  /* TEXT_PLAIN => nothing */
	1,  /* NEEDS_ESCAPE => TEXT_PLAIN */
};
#endif /* YYFALLBACK */
...
struct yyParser {
	...
	#endif
	TDPParseARG_SDECL                /* A place to hold %extra_argument */
	#if YYSTACKDEPTH<=0
...
static const char *const yyTokenName[] = {
	"$",             "TEXT_PLAIN",    "NEEDS_ESCAPE",  "RECORD_DELIMITER",
	"TDP_EOF",       "FIELD_DELIMITER",  "ESCAPED_ESCAPE",  "TEXT_NUMERIC",
	"ESCAPE",        "error",         "doc",           "header",
	"records",       "eol",           "record",        "fields",
	"field",         "contents",      "content",       "escaped_contents",
	"escaped_content",
};
#endif /* NDEBUG */
...
** second argument to TDPParseAlloc() below.  This can be changed by
...
static void yyStackOverflow(yyParser *yypParser) {
	...
	** stack every overflows */
	...
	TDPParseARG_STORE; /* Suppress warning about unused %extra_argument var */
}
...
static void yy_shift(
	...
	TDPParseTOKENTYPE yyMinor        /* The minor token to shift in */
) {
	yyStackEntry *yytos;
	yypParser->yytos++;
	#ifdef YYTRACKMAXSTACKDEPTH
...
static void yy_reduce(
	...
	** enough on the stack to push the LHS value */
	...
	switch ( yyruleno ) {
		...
		case 0: { /* doc ::= header records */
			*result = yymsp[-1].minor.yy0;
		...
		case 2: { /* header ::= record */
			yymsp[0].minor.yy0->type = TDP_HEADER;
		}
		break;

		case 3: /* records ::= records record */
		...
		case 4: { /* record ::= fields eol */
			yylhsminor.yy0 = simple_token_new_parent(yymsp[-1].minor.yy0, TDP_RECORD);
		...
		case 6: { /* field ::= contents FIELD_DELIMITER */
			yylhsminor.yy0 = simple_token_new_parent(yymsp[-1].minor.yy0, TDP_FIELD);
			simple_token_free(yymsp[0].minor.yy0);

			if (yymsp[-1].minor.yy0->type == TEXT_NUMERIC && yymsp[-1].minor.yy0->next == NULL) {
				yylhsminor.yy0->type = TDP_FIELD_NUMERIC;
			}
		}
		...
		case 7: { /* field ::= contents */
			yylhsminor.yy0 = simple_token_new_parent(yymsp[0].minor.yy0, TDP_FIELD);

			if (yymsp[0].minor.yy0->type == TEXT_NUMERIC && yymsp[0].minor.yy0->next == NULL) {
				yylhsminor.yy0->type = TDP_FIELD_NUMERIC;
			}
		}
...
static void yy_syntax_error(
	...
	TDPParseTOKENTYPE yyminor         /* The minor type of the error token */
) {
	TDPParseARG_FETCH;
...
** "TDPParseAlloc" which describes the current state of the parser.
...
void TDPParse(
	...
	TDPParseTOKENTYPE yyminor       /* The value for the token */
	TDPParseARG_PDECL               /* Optional %extra_argument parameter */
) {
